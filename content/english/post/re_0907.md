...
Therefore, the indirect Scope 2 emissions of one unit of the final output for sector i in country s are $$\color{blue} f^{s,l}_i = f^{s,D} * \sum_r a^{s,r}_{i,D}$$, where $f^{s,d}$, $f^{s,d}_D$
