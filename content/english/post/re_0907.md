...
Therefore, the indirect Scope 2 emissions of one unit of the final output for sector i in country s are $$\color{blue} f^{s,l}_i = f^{s,D35E36} × \sum_r a^{s,r}_{i,D35E36}$$, where $\color f^{s,d} $, $\color f^{s,d}_D$
represents the direct emissions from one unit of total output of the energy sector in the exporting country, $\color a_{a,D35E36}$ is the direct consumption coefficient of sector 𝑖
