...
Timber houses are generally considered an environmentally friendly substitution for steel and concrete structures for two reasons: firstly, using timber can avoid considerable emissions during material production processes[^39]; secondly, timber can serve as long-term carbon storage if sources sustainably[^40]. However, there are also concerns surrounding land-use change to manage forests and related biodiversity
